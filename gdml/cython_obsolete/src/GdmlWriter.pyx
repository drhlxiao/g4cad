...
class GdmlWriter(object):
    ...
    def __init__(self):
        ...
        self.db=MaterialDatabase.MaterialDatabase()
    ...
    def processMaterial(self, mat):
        if mat in G4Materials.materials:
            self.printf('Geant4 default material: '+mat)
        else:
            self.printf('Loading material'+mat+ ' information from database')
    ...
    def addPosition(self, name, x, y, z,unit="cm"):
        self.define[2].append(['position',{'name':name, 'x':x, 'y':y, 'z':z,'unit':unit},[]])
    ...
    def computeRotationDefineByAxis(self,xaxis,yaxis,zaxis):
        ...
        if (r>1.e-6):
            ...
            if (angle_z<1e-6 and angle_z>-1e-6):
                angle_z=0.
            y_1=[0.,cos_z*yaxis[1]+sin_z*xaxis[1],cos_z*yaxis[2]+sin_z*xaxis[2]]
            x_1=[cos_z*xaxis[0]-sin_z*yaxis[0],cos_z*xaxis[1]-sin_z*yaxis[1],cos_z*xaxis[2]-sin_z*yaxis[2]]
            z_1=zaxis
    ...
    def addRotation(self, name, x, y, z,unit='deg'):
        self.define[2].append(['rotation',{'name':name, 'x':x, 'y':y, 'z':z, 'unit':unit},[]])
    ...
    def addMaterial(self, name,formula, Z,atom, D,unit="g/cm3"):
        if name not in self.__materials:
            self.materials[2].append(['material', {'name':name, 'Z':Z,"formula":formula},
                                               [['D',{'value':D,'unit':unit},[]], ['atom',{'value':atom},[]]] ] )
    ...
    def addIsotope(self, name, Z, N, a):
        if name not in self.__isotops:
            self.materials[2].append(['isotope', {'name':name, 'N':N, 'Z':Z },
                [['atom', {'value':a, "type":"A" },[]] ]])
    ...
    def addElements(self, name, isotopedict):
        if len(isotopedict)<1:
            return False;
        isotopes=[]
        for reference,fraction in isotopedict.iteritems():
            isotopes.append(['fraction',{'ref':reference, 'n':fraction},[]])
    ...
    def addWorld(self,name='world',x=10000,y=1000,z=10000):
        ...
        return name
    def createWorldVolume(self,x=10000,y=10000,z=10000,world_mat='G4_Galactic'):
        if self.world_volume_name=="":
            solid_world=self.addWorld('__box_world__',x,y,z)
            self.world_volume_name=self.addVolume('world',solid_world,world_mat, daughters=[])
    ...
    def addBox(self, name, dx, dy, dz,lunit="cm"):
        ...
        self.solids[2].append(['box',{'name':name, 'x':dx, 'y':dy, 'z':dz, 'lunit':lunit},[]])
    def box(self, name, dx, dy, dz,lunit="cm"):
        self.addBox(name, dx, dy, dz,lunit)
        return [dx,dy,dz]
    def addSphere(self, name, rmin, rmax, startphi, deltaphi, starttheta, deltatheta,lunit="cm",aunit="deg"):
        self.solids[2].append(['sphere',{'name':name, 'rmin':rmin, 'rmax':rmax,
                                         ...
                                         'aunit':aunit, 'lunit':lunit},[]])

    def addCone(self, name, z, rmin1, rmin2, rmax1, rmax2, sphi, dphi,lunit="cm",aunit="deg"):
        self.solids[2].append(['cone',{'name':name, 'z':z, 'rmin1':rmin1, 'rmin2':rmin2,
                                       'rmax1':rmax1, 'rmax2':rmax2,
                                       'startphi':sphi, 'deltaphi':dphi,
                                       'aunit':aunit, 'lunit':lunit}, []] )

    def cone(self,name,rmin1, rmax1, rmin2,rmax2,z,sphi,dphi,lunit="cm",aunit="deg"):
        self.addCone(name, z, rmin1, rmin2, rmax1, rmax2, 
                     sphi, dphi,lunit,aunit)
    ...
    def addTube(self, name, rmin, rmax, z, startphi, deltaphi,lunit="cm",aunit="deg"):
        self.solids[2].append(['tube',{'name':name, 'rmin':rmin, 'rmax':rmax,
                                       ...
                                       "aunit":aunit,"lunit":lunit},[]])
    def tube(self, name, rmin, rmax, z, startphi, deltaphi,lunit="cm",aunit="deg"):
    ...
    def addXtru(self, name, xy_polygon, z_pos_vec,x_offset_vec,y_offset_vec,scaling_vec,unit="cm"):
        polygon_vertices=[]
        for vertex in xy_polygon:
            polygon_vertices.append( ['twoDimVertex',{'x':vertex[0], 'y':vertex[1]},[]] )
        z_sections=[]
        for i in range(len(z_pos_vec)):
            ...
            z_sections.append( ['section',{'zOrder':i+1, 
                                           'zPosition':z,'xOffset':x_off,
                                           'yOffset':y_off,'scalingFactor':scaling},[]])
        self.solids[2].append( ['xtru', {'name':name, 'lunit':unit},polygon_vertices+z_sections] )
    def addPos(self, subels, type, name, v,unit="m"):
        if v[0]!=0.0 or v[1]!=0.0 or v[2]!=0.0:
            subels.append( [type,{'name':name, 'x':v[0], 'y':v[1], 'z':v[2],'unit':unit},[]] )

    def addRot(self, subels, type, name, v,unit="deg"):
        if v[0]!=0.0 or v[1]!=0.0 or v[2]!=0.0:
            subels.append( [type,{'name':name, 'x':v[0], 'y':v[1], 'z':v[2], 'unit':unit},[]] )

    def addBoolean(self, type_boolean,name, first_name, second_name, translation, rotation,
                   lunit="cm",aunit="deg"):
        subels = [['first',{'ref':first_name},[]],
                ['second',{'ref':second_name},[]]]
    ...
    def addPhysVolume(self, mother_name,log_vol_name, phys_vol_name, position_name,rot_name=None):
        
        mother_vol=self.getLogicalVolume(mother_name)

        phys_vol_elem=["physvol",{},[["volumeref",{"ref":log_vol_name},[]],
    ...
    def includePhysVolume(self, mother_name,phys_vol_name, file_name, pos,rot):
        ...
        if pos:
            ...
            self.addPosition(position_name, pos['x'],pos['y'],pos['z'],pos['unit'])
        if rot:
            rot_name=rot['name']
            self.addRotation(rot_name, rot['x'],rot['y'],rot['z'],rot['unit'])
    ...
    def addSetup(self, name, version, world):
        ...
        self.document[2].append( ['setup',{'name':world_name, 'version':version},
                                   [ ['world',{'ref':world},[]]] ] )
        return world_name
        ...
        def writeElement(elem, offset):
            ...
            for attr in ordered_keys:
                if attr in self.list_float_elem :
                    file.write(' %s="%.6e"' %(attr,float(elem[1][attr])))
                else:
    ...
    def getPosition(self, name):
        posElem=self.getXmlElement(self.define,
                                'position',name)
    ...
    def getRotation(self, name):
        rotElem=self.getXmlElement(self.define,
                                'rotation',name)
    ...
    def removePosition(self, name):
        return self.removeElement(self.define,
                                'position',name)
                    ...
                    if name_phys_vol is None:
                        for subelem in elem[2]:
                            if subelem[0] == 'volumeref':
                                name_phys_vol= self.getAttributeValue(subelem,'ref',)
        ...
        for vol in self.structure[2]:
            ...
            for elem in vol[2]:
                if elem[0] == "physvol":
                    ...
                    for subelem in elem[2]:
                        if subelem[0] == 'volumeref':
                            name_log_vol=self.getAttributeValue(subelem,'ref',)
                    if (name_phys_vol is None):
            ...
            for elem in vol[2]:
                if elem[0] == "physvol":
                    ...
                    if name_phys_vol is None:
                        for subelem in elem[2]:
                            if subelem[0] == 'volumeref':
                                name_phys_vol= self.getAttributeValue(subelem,'ref')+"_PV"
                    ...
                    if elem[0] == "physvol":
                        ...
                        for subelem in elem[2]:
                            ...
                            if subelem[0] == 'position':
                                unit=eval(self.getAttributeValue(subelem,'unit','mm'))
                                x=eval(self.getAttributeValue(subelem,'x',0.))*unit
                                y=eval(self.getAttributeValue(subelem,'y',0.))*unit
                                z=eval(self.getAttributeValue(subelem,'z',0.))*unit
                                pos=np.array([x,y,z])
